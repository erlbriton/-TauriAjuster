...
export interface ISerialPort {
    ...
    /**
     * Задать путь к порту ПЕРЕД вызовом connect (только нативная реализация).
     * Необязательный метод (знак "?"): браузерная реализация выбирает порт
     * сама через диалог, поэтому у неё этого метода может не быть.
     */
